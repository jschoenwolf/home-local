...
class Jgs_IMDb
{
    ...
    function __construct($anonymise = false, $summary = true, $titlesLimit = 0)
    {
        ...
        if (intval($titlesLimit) > 0)
            $this->titlesLimit = intval($titlesLimit);  // Set titles limit if required
    }
            ...
            foreach ($list as $obj) {
                if (!empty($obj->tconst) AND !$this->is_ignored($obj->type) AND $this->yearMatch($obj->year, $yearMatch)) {
                    ...
                    if (is_array($obj->principals)) {
                        foreach ($obj->principals as $cast) {
                            $actor[] = $cast->name;
                        }
    ...
    function is_ignored($type, $cert = "", $genre = array())
    {
        ...
        if (in_array($type, $this->ignoreTypes))
            return true;
        if ($this->ignoreAdult AND ($cert == "X" OR (!empty($genre) AND in_array("Adult", $genre))))
            return true;
